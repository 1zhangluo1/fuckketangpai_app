...
class ShortAnswerQuestion extends StatefulWidget {
  const ShortAnswerQuestion({super.key, required this.question, required this.courseId, required this.testPaperId});
...
class _ShortAnswerQuestionState extends State<ShortAnswerQuestion> {
  ...
  void initState() {
    ...
    widget.question.answerStringContents.asMap().forEach((index, e) {
      answerController.text += (e + (index < widget.question.answerStringContents.length - 1 ? '\n' : ''));
    });
  }
